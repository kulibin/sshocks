...
// Server is a minimal HTTP/1.1 CONNECT proxy.
type Server struct {
...
// NewServer creates an HTTP CONNECT proxy bound to listen, dialing targets
// through the provided Dialer. Sessions are tracked so Drain can wait /
// force-close them.
func NewServer(listen string, dialer Dialer, l log.Logger) *Server {
	ln, err := net.Listen("tcp", listen)
	if err != nil {
		if l != nil {
			l.Errorf("httpproxy: listen %s failed: %v", listen, err)
		}
...
func (s *Server) Run(ctx context.Context) error {
	...
	for {
		...
		if err != nil {
			...
			default:
				if s.log != nil {
					s.log.Warnf("httpproxy: accept error: %v", err)
				}
				return err
			}
		}
...
func (s *Server) Drain(timeout time.Duration) {
	if s.log != nil {
		s.log.Infof("httpproxy: draining sessions (timeout %s)...", timeout)
	}
	_ = s.Shutdown()
	...
	case <-done:
		if s.log != nil {
			s.log.Infof("httpproxy: drain complete, all sessions closed")
		}
	case <-time.After(timeout):
		if s.log != nil {
			s.log.Warnf("httpproxy: drain timeout reached, forcing close of remaining sessions")
		}
		s.cancel()
	}
}
